...
The efficiency of any artificial intelligence model is heavily dependent on the quality and structure of the data used for training. In this work, the data originates from customer samples sequenced using the ABI3730xl sequencer. The sequencer outputs a variety of files per plate well, including raw fluorescence data, the processed sequence itself, and additional quality metrics.
Each PCR plate generates a significant volume of data, with millions of files produced over time, as multiple plates are processed each day. Since each PCR plate contains 96 wells, and each well generates 7 files, this results in 672 files per plate. 
In this work, the files taken into account are the following:
...
The database thus contains Dailybooks for each day, with reactions processed during that specific day
