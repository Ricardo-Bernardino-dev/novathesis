...
\subsection{Step 1: BLAST-like neighbourhoods vs.\ DBSCAN@$k$-mer}
\begin{table}[H]
\centering
...
\end{tabular}
\end{table}
% Optionally include a figure with the plate layout graph.
...
\end{tabular}
\end{table}
% Optionally include example AE loss curves per plate.
...
\end{tabular}
\end{table}
% Add a figure with permutation importance (w/ and w/o Client_ID/Primer_ID).

\subsection{Step 4: Rule fusion}
\begin{table}[H]
\centering
...
\end{tabular}
\end{table}
% Optionally include PR curves with/without fusion.
